...
export const DataGridRow = React.memo(DataGridRowImpl, (prev, next) => {
  ...
  // Re-render if table meta changed (token selection, draft revision, etc.)
  ...
  if (prev.row.getIsSelected() !== next.row.getIsSelected()) {
    return false;
  }
...
function DataGridRowImpl<TData>({
  ...
  const selectionMode = tableMeta.selectionMode ?? "cell";
  ...
  const visibleCells = React.useMemo(
    () => row.getVisibleCells(),
    [row, row.original, columnVisibility, columnPinning],
  );
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isRowSelectionMode && isRowSelected && "bg-primary/5",
        className,
      )}
      style={{
    ...
    >
      {visibleCells.map((cell, colIndex) => {
        ...
        const isCellSelected =
          isRowSelectionMode && isRowSelected
            ? true
          ...
          >
            {typeof cell.column.columnDef.header === "function" ? (
              ...
              <DataGridCell
                ...
                columnId={columnId}
                rowHeight={rowHeight}
                isFocused={isRowSelectionMode ? false : isCellFocused}
