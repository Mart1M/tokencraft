...
function areKeyValueItemsEqual(a: ItemData[], b: ItemData[]) {
...
function KeyValue(props: KeyValueProps) {
  ...
  const onFormTriggerChange = React.useCallback((node: RootElement | null) => {
    setFormTrigger((current) => (Object.is(current, node) ? current : node));
  }, []);
  const composedRef = useComposedRefs(ref, onFormTriggerChange);
    ...
    return {
      ...
      setState: (key, val) => {
        if (key === "value" && Array.isArray(val)) {
          const nextValue = val as ItemData[];
          if (areKeyValueItemsEqual(stateRef.current.value, nextValue)) {
            return;
          }
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (areKeyValueItemsEqual(stateRef.current.value, valueProp)) {
      return;
    }

    stateRef.current.value = valueProp;
    store.notify();
  }, [valueProp, stateRef, store]);
