...
function useDataGrid<TData>({
  ...
  const selectRange = React.useCallback(
    (start: CellPosition, end: CellPosition, isSelecting = false) => {
      const isRowSelectionMode = propsRef.current.selectionMode === "row";
      const firstColumnId = columnIds[0];
      const lastColumnId = columnIds[columnIds.length - 1];

      const startColIndex = isRowSelectionMode
        ? 0
        : columnIds.indexOf(start.columnId);
      const endColIndex = isRowSelectionMode
        ? Math.max(columnIds.length - 1, 0)
        : columnIds.indexOf(end.columnId);
      ...
      const normalizedStart =
        isRowSelectionMode && firstColumnId
          ? { rowIndex: start.rowIndex, columnId: firstColumnId }
          : start;
      const normalizedEnd =
        isRowSelectionMode && lastColumnId
          ? { rowIndex: end.rowIndex, columnId: lastColumnId }
          : end;
  ...
  const onCellClick = React.useCallback(
    (rowIndex: number, columnId: string, event?: React.MouseEvent) => {
      ...
      if (propsRef.current.selectionMode === "row") {
        const rows = tableRef.current?.getRowModel().rows ?? [];
        const row = rows[rowIndex];
        const firstColumnId = columnIds[0];
        const lastColumnId = columnIds[columnIds.length - 1];
        if (!row || !firstColumnId || !lastColumnId) return;

        if (event?.shiftKey) {
          const lastClickedRowId = store.getState().lastClickedRowId;
          const lastRowIndex = lastClickedRowId
            ? rows.findIndex((candidate) => candidate.id === lastClickedRowId)
            : -1;
          const startRowIndex = lastRowIndex >= 0 ? lastRowIndex : rowIndex;

          selectRange(
            { rowIndex: startRowIndex, columnId: firstColumnId },
            { rowIndex, columnId: lastColumnId },
          );
          ...
          for (const [rowId, selected] of Object.entries(newRowSelection)) {
            if (!selected) continue;

            const selectedRowIndex = rows.findIndex(
              (candidate) => candidate.id === rowId,
            );
            if (selectedRowIndex === -1) continue;

            for (const colId of columnIds) {
              selectedCells.add(getCellKey(selectedRowIndex, colId));
            }
          }
          ...
          store.batch(() => {
            ...
            store.setState("focusedCell", null);
            store.setState("editingCell", null);
            store.setState("lastClickedRowId", row.id);
          });
        } else {
          selectRange(
            { rowIndex, columnId: firstColumnId },
            { rowIndex, columnId: lastColumnId },
          );
          store.setState("lastClickedRowId", row.id);
        }
  ...
  const onCellDoubleClick = React.useCallback(
    (rowIndex: number, columnId: string, event?: React.MouseEvent) => {
      ...
      if (propsRef.current.selectionMode === "row") return;

      onCellEditingStart(rowIndex, columnId);
    },
    [onCellEditingStart, propsRef],
  );
  ...
  const onCellMouseDown = React.useCallback(
    ...
    [store, propsRef, columnIds, selectRange],
  );
  ...
  const onCellMouseEnter = React.useCallback(
    ...
    [store, propsRef, selectRange, focusCell],
  );
  ...
  const tableMeta = React.useMemo<TableMeta<TData>>(() => {
    return {
      ...
      get selectionMode() {
        return propsRef.current.selectionMode ?? "cell";
      },
      getIsCellSelected,
      getIsSearchMatch,
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    measureElement: !isFirefox
      ...
      : undefined,
    scrollPaddingStart: VIEWPORT_OFFSET,
