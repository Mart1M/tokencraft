...
import {
  ...
  renameWorkspaceTokenFile,
  WorkspaceFsError,
  writeWorkspaceRawFiles,
...
export async function POST(request: Request) {
  ...
  if (
    ...
    !input.collectionCreates.length &&
    !input.folderCreates.length &&
    !input.collectionRenames.length &&
    !input.folderRenames.length &&
    !input.modeChanges.length &&
    !input.fileWrites.length
  ) {
  ...
  try {
    ...
    if (input.fileWrites.length) {
      const result = await writeWorkspaceRawFiles(input.rootPath, input.fileWrites);
